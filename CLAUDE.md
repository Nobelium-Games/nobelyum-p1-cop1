...
- **`DialogueData.cs`** — ScriptableObject. `DialogueID`, `List<DialogueNode> Nodler`.
  ...
  SonrakiNodeID, `List<StatEtkisi> StatEtkileri` (her biri bir `StatAdi` + `Miktar` çifti — bir seçenek
  artık birden fazla stat'ı aynı anda etkileyebiliyor).
...
- **`DialogueManager.cs`** — Aktif diyaloğu ekrana basıyor: `NpcIsimText`, `NpcSozuText`,
  `SecenekButon1Text`, `SecenekButon2Text` (TMP_Text referansları), `PortreImage` (Image, NPC portresi),
  `SecenekButon2` (GameObject — tek seçenekli diyaloglarda `SetActive(false)` ile gizleniyor, örn. elçi
  diyaloğunda), `SecenekButon1Buton`/`SecenekButon2Buton` (Button referansları — `SecenekKarsilanabilirMi`
  kontrolüne göre `interactable` ayarlanıyor). `DiyalogBaslat(DialogueData diyalog, Sprite portre, string isim)`,
  `Secenek1Secildi()`, `Secenek2Secildi()`. Bir seçenek seçilince her `StatEtkisi` için hem
  `GameManager.Instance.State.StatDegistir` hem `BildirimYoneticisi.Instance.Bildirim` çağrılıyor.
  Diyalog bitince `DayCycleManager.Instance.SiradakiyeGec()` çağırıyor. Doğrulandı, sorun yok.
- **`GunUI.cs`** — Ekranın üstünde `GameManager.Instance.State.Gun`'ı "Gun X" olarak gösteriyor (TMP_Text).
  Test edildi, sorun yok.
- **`BildirimYoneticisi.cs`** — Singleton. `BildirimSablonu` (bir TMP_Text objesi, Unity'de
  `Vertical Layout Group` + `Content Size Fitter` olan bir panelin altında duruyor, `Awake`'te
  `SetActive(false)` ile gizleniyor). `Bildirim(statAdi, miktar)`: şablonu `Instantiate` ile çoğaltıp
  (+yeşil/-kırmızı renkli) birkaç saniye gösterip `Coroutine` ile yok ediyor. Test edildi, sorun yok.
...
- **NPC portre/karakter görseli sistemi eklendi ve test edildi:** `NPCData.cs`'de `Portre` (Sprite)
  ...
  referansı var, `DiyalogBaslat(diyalog, portre, isim)` artık portreyi de alıp ekrana basıyor.
  `DayCycleManager.cs`, `Dialog.DiyalogBaslat(npc.Diyalog, npc.Portre, npc.Isim)` şeklinde çağırıyor.
  Not: Ulak (elçi/gece sonucu) NPC'sinin runtime'da oluşturulan `NPCData`'sında portre atanmadığı için
  o anda Image boş kalıyor — bilinçli, sorun değil.
- **`Gun` (gün sayacı) eklendi ve test edildi:** `GameState.Gun` (1'den başlıyor), `DayCycleManager.UyuyaBas()`
  içinde her Uyu'ya basışta +1 artıyor. `GunUI.cs` (yeni script) ekranın üstünde "Gun X" olarak gösteriyor.
- **10. gün sabit hikaye NPC'si (Ayyaş Adam) eklendi:** `DaySequencer.SiradakiListeyiOlustur`'a yeni
  `ayyasNpc` parametresi + "Kural 0: Gun == 10 ise ayyasNpc ekle" kuralı eklendi. `DayCycleManager.cs`'te
  `AyyasNpc` (NPCData) alanı var, Inspector'dan `Ayyas_NPC` asset'i atanmış, test edildi.
  `DaySequencer` artık `(state, koyluNpc, askerNpc, ayyasNpc)` imzasıyla çağrılıyor — yeni sabit-günlü
  NPC eklenecekse bu fonksiyona benzer şekilde yeni parametre + `Gun == X` kuralı eklenmeli.
- **Diyalog seçenekleri artık birden fazla stat'ı aynı anda etkileyebiliyor:** `DialogueData.cs`'te eski
  `EtkilenenStat`/`StatDegisimi` alanları kaldırıldı, yerine `DialogueChoice.StatEtkileri`
  (`List<StatEtkisi>`, her biri `StatAdi` + `Miktar`) geldi. `DialogueManager.SecenekUygula` artık
  listedeki her etkiyi `foreach` ile sırayla uyguluyor. **Not:** Bu değişiklik mevcut diyalog
  asset'lerindeki eski tek-stat verilerini sıfırladı, kullanıcı Inspector'dan yeniden girdi.
- **Diyalog seçenekleri artık "karşılanabilirlik" kontrolü yapıyor:** `DialogueManager.SecenekKarsilanabilirMi`
  bir seçenek herhangi bir stat'ı eksiye düşürecekse `false` dönüyor. `NodeGoster()` her node gösterildiğinde
  bu kontrolle `SecenekButon1Buton`/`SecenekButon2Buton` (yeni `Button` referansları, Inspector'dan atandı)
  `.interactable`'ını ayarlıyor — yetersiz kaynak gerektiren seçenek buton olarak görsel kilitleniyor.
- **Gece sonuç mesajları renklendirildi (`DayResolver.cs`):** Şansa bağlı (zar atılan) sonuçlar
  `<color=green>...</color>` (başarılı) / `<color=red>...</color>` (başarısız); şansa bağlı OLMAYAN
  (garanti) tamamlanma mesajları `<color=yellow>...</color>`. Çok günlü emrin "başladı" mesajı kaldırıldı
  (gereksiz — oyuncu zaten butona bastığını biliyor), "devam ediyor, X gün kaldı" mesajı renksiz/beyaz
  kalıyor. Mesajlar `TMP_Text` üzerinden gösterildiği için rich-text renk etiketleri direkt çalışıyor.
- **Diyalog panelinde NPC ismi gösteriliyor:** `DialogueManager.cs`'te yeni `NpcIsimText` (TMP_Text) alanı,
  `DiyalogBaslat` artık `isim` parametresi de alıp bu alana yazıyor.
- **Anlık bildirim (toast) sistemi eklendi:** Yeni `BildirimYoneticisi.cs` (Singleton, Mount & Blade
  Warband tarzı sağ-alt bildirim yığını). `Bildirim(statAdi, miktar)` fonksiyonu bir şablon objeyi
  (`BildirimSablonu`, Unity'de `BildirimPanel` altında, `Vertical Layout Group` + `Content Size Fitter`
  ile otomatik diziliyor) çoğaltıp (+/- rengiyle) birkaç saniye gösterip `Coroutine` ile kendini yok
  ediyor. `DialogueManager.SecenekUygula`, her `StatEtkisi` uygulandığında bunu çağırıyor. Test edildi,
  çalışıyor.
...
"Gerçek bir Danışman Paneli yapalım" planı tamamlandı (buton görsel kilitlenmesi + `ManagerTest.cs` →
`DanismanPaneli.cs` yeniden adlandırma). Panelin görsel düzenini (ikon, layout) iyileştirme adımı henüz
ele alınmadı, gündeme gelirse detay kullanıcıya sorulmalı.

Bu oturumda kullanıcı UI yerine önce oyun mekaniklerini/daha fazla stat'ı genişletmeye karar verdi.
**Yeni oturum burada devam edebilir, açık bir "sıradaki adım" yok** — kullanıcı ne istediğini söyleyecek.
Şu an akla gelebilecek doğal devamlar (henüz istenmedi, sadece olası yönler):
- `DaySequencer.cs`'ye Ayyaş Adam (10. gün) gibi başka sabit-günlü hikaye NPC'leri eklemek (aynı pattern:
  yeni parametre + `Gun == X` kuralı).
- Daha fazla diyalog/NPC içeriği (yeni `StatEtkileri` yapısıyla).
- Kaydet/yükle (save/load) sistemi — `Gun` eklendiği için artık kaydedilecek daha fazla state var.
- Panelin görsel düzeni (yukarıdaki yarım kalan adım).
