...
- **`ManagerTest.cs`** — **HÂLÂ GEÇİCİ/TEST script'i, kalıcı mimarinin parçası değil.** Şahsi Oda
  butonlarına (İnşaatçı, Askerbaşı, Asker Topla) sabit `OrderData`'lar oluşturup `OrderManager.EmirEkle`'ye
  gönderiyor: Değirmen İnşa Et (Altın maliyetli, tamamlanınca Erzak base gelirini +1 artırıyor), Köy
  Yağmala (Manpower maliyetli, şansa bağlı), Asker Topla (Altın maliyetli, garantiye yakın, Manpower +10
  veriyor — Köy Yağmala ile aynı danışmanı paylaşıyor). **Şu an aktif olarak "gerçek Danışman Paneli"ne
  dönüştürülüyor, bkz. "Sıradaki Adımlar".**
...
- **`DanismanButon.cs`** — **YENİ, henüz test edilmedi (bkz. "Sıradaki Adımlar").** Bir danışman butonuna
  eklenip `Update()` içinde `Orders.DanismanKullanildiMi(DanismanTipi)`'a bakarak `Button.interactable`'ı
...
1. ✅ *(kod yazıldı, ama henüz Unity'de test edilip onaylanmadı — bir sonraki oturuma BURADAN başla)*
   Butonların görsel kilitlenmesi: `DanismanButon.cs` oluşturuldu. Kullanıcıya şu adımlar söylendi ama
   sonucu doğrulanmadı:
   - `InsaatciButon`'a `DanismanButon` component'i eklenip `Danisman Tipi = "Insaatci"`, `Orders =`
     (OrderManager objesi) atanacak.
   - `AskerbasiButon`'a aynısı, `Danisman Tipi = "Askerbasi"`.
   - `AskerToplaButon`'a aynısı, `Danisman Tipi = "Askerbasi"` (Köy Yağmala ile aynı danışmanı paylaşıyorlar).
   - Test: bir danışmana emir verilince o butonun (ve aynı danışmanı paylaşan diğer butonun) görsel
     olarak soluklaşıp tıklanamaz hale geldiğini, "Uyu" sonrası yeniden aktif olduğunu doğrulamak gerekiyor.
   **Yeni oturum önce bu adımın test sonucunu sormalı** ("DanismanButon'u Unity'de bağladın mı, çalıştı mı?").
2. ⬜ Henüz yapılmadı: `ManagerTest.cs`'i kalıcı bir `DanismanPaneli` script'ine dönüştürmek (isim/mimari
   değişikliği — `ManagerTest` adı ve "geçici test script'i" statüsü kalkacak).
3. ⬜ Henüz yapılmadı: Panelin görsel düzenini iyileştirmek (ikon, layout, vs. — henüz detay konuşulmadı).
