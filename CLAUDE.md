...
`AltinYield`'i, `Nufus`'u (bkz. "Nüfus Mekaniği"), `Savunma`'sı (isyan bastırma başarı hesabında
kullanılıyor, bkz. altta), `MaxBinaSlotu`/`DoluBinaSlotu` (bina sınırı) ve `IsyanHalinde` (isyan
durumu) var.
...
- **`KoyData.cs`** — Bir köyün "kimlik kartı": `Isim`, `Sadakat` (varsayılan 50), `Erzak`
  ...
  **`Savunma`** (varsayılan 20, isyan bastırma başarı formülünde kullanılıyor, bkz. Mimari
  Kararları #17), `MaxBinaSlotu` (varsayılan 3), `DoluBinaSlotu` (varsayılan 0), **`IsyanHalinde`**
  (bool, varsayılan false). ScriptableObject DEĞİL.
...
- **`KoyYoneticisi.cs`** — Singleton, `public List<KoyData> Koyler`. `Awake()`'te artık her köyün
  ...
  `ToplamAltinYieldi()`, `ToplamNufus()`, `NufusDegistir(miktar)` (bu dördü/beşi artık isyandaki
  köyleri atlıyor), `OrtalamaSadakat()`, **`IsyanKontrolEt(mesajListesi)`** (gece isyan zarı, bkz.
  Mimari Kararları #14). **`NufusYieldHesapla(koy)`**, **`NufusuGunlukArtir()`**, **`ToplamNufusYieldi()`**
  *(bu oturumda eklendi)* — Nüfus büyüme formülü, bkz. Mimari Kararları #18. `NufusEsik`
  (varsayılan 1) ve `NufusKatsayi` (varsayılan 10), Inspector'dan ayarlanabilir public alanlar.
...
- **`KoyBilgiPaneli.cs`** *(yeni)* — Singleton. `Panel`, `IsimText`, `DurumText` (isyan varsa kırmızı
  ...
  ("1/3" formatı), `SavunmaText`. **`NufusText`** *(bu oturumda eklendi)* — "Nufus: X <sup>+Y</sup>"
  formatında, Y `KoyYoneticisi.NufusYieldHesapla(koy)` ile canlı hesaplanıyor (StatsUI ile aynı
  desen, ayrı bir "Nufus Yield" satırı YOK, üst indis olarak gösteriliyor). `Goster(KoyData koy)` / `Kapat()`.
...
- **`StatsUI.cs`** — Sol üstteki canlı stat göstergesi. Erzak `KoyYoneticisi.ToplamErzak()`'tan,
  ...
  (isyandaki köyler otomatik 0 sayılıyor), Sadakat genel+köy ortalaması, **Nüfus** `ToplamNufus()`'tan
  (bu oturumda eklendi). Üst indis gelir göstergeleri (`GelirMetni` yardımcı fonksiyonu, bu oturumda
  eklendi) artık +yeşil/-kırmızı/0-beyaz renkli (Köy Bilgi Paneli'ndeki `YieldMetni` ile aynı desen).
...
6. ✅ İsyan bastırılınca hedef köyün Sadakat'ı da +10 artıyor (aşağıdaki maddeye kısmi cevap).
7. ✅ Nüfus sistemi eklendi — kingdom Nüfus'u köylerin toplamı, günlük büyüme köyün Erzak stokuna
   bağlı dinamik formülle hesaplanıyor (`KoyData.Nufus`, `KoyYoneticisi.NufusYieldHesapla`,
   `NufusEsik`/`NufusKatsayi` Inspector'dan ayarlanabilir), StatsUI ve Köy Bilgi Paneli'nde gösteriliyor.
8. ✅ StatsUI'deki üst indis gelir göstergeleri renklendirildi (+yeşil/-kırmızı/0-beyaz).
